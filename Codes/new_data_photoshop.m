...
load PhotoshopPredictFull.csv
load PhotoShopPredictNewLabels.csv
load PhotoshopPredict3.csv


%grp_idx = grp2idx(FeatureLabels);

X = PhotoshopPredictFull(1:23,:);
y = PhotoShopPredictNewLabels(1:23,:);
X_new_data = PhotoshopPredict3(1:end,:);

%dividing the dataset into training and testing 
rand_num = randperm(23);

%training Set
X_train = X(rand_num(1:19),:);
y_train = y(rand_num(1:19),:);

%testing Set
X_test = X(rand_num(20:end),:);
y_test = y(rand_num(20:end),:);
...
if c == 0
    frprintf('Non Resampled')
end

saveCompactModel(SVMModel,'SVMIonosphere');
