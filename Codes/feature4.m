...
for i = 1:nob
  ...
  %probabilityMap = emresample(img,N,'verbose') 
  freqDomMap = fft2(probabilityMap)
  
      localEnergyVal = 0;
      localEnergy = freqDomMap.*conj(freqDomMap);
       
       disp('Local energy :');
       %disp(energy);
       for plength = 1: length(localEnergy)
            
           localEnergyVal = localEnergyVal + localEnergy(plength);
           
       end
        localenergymapRow = reshape(localEnergy, 1, []);
        [sortedLocalVal, sortedInds] = sort(localenergymapRow,'descend');
         topLocalVal = sortedLocalVal(1:1);
         fprintf('max value :');
         disp(topLocalVal);
        
         f4= (topLocalVal / localEnergyVal);
