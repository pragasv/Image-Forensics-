...
im  = imread('Original/ucid/ucid00660.tif');
im = rgb2gray(im);
[H,W,~] = size(im);

nob=(H/64)*(W/64);
   Blocks = cell(H/64,W/64);
   counti = 0;
for i = 1:64:H-63
   counti = counti + 1;
   countj = 0;
   for j = 1:64:W-63
        countj = countj + 1;
        Blocks{counti,countj} = im(i:i+63,j:j+63);
        
        pmap = emresampleN(im,N,'verbose')
...
dlmwrite('featuresphoto.csv',feature_arrayO,'-append');
